    ...
    <form action="{{ route('campaigns.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="4"
                      class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:border-purple-500 focus:ring-purple-500 sm:text-sm">{{ old('description') }}</textarea>
        </div>

        <div class="pt-4 border-t flex justify-between">
            <a href="{{ route('campaigns.index') }}" class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300">
                Cancel
            </a>
            <button type="submit"
